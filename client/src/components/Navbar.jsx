...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-4'>
        <Link to='/'>
        <img src={logo} alt="logo" className='w-28 sm:w-22 lg:w-40'/>
        </Link>
        ...
        <div>
            ...
            <div className='flex items-center gap-2 sm:gap-3'>
                <button onClick={()=>navigate('/buy')} className='flex itmes-center gap-2 bg-blue-100 px-4 sm:px-6 py-1.5 sm:py-3 rounded-full hover:scale-105 transition-all duration-100'>
                    <img className="w-5" src={credit_star} alt="" />
                    <p className='text-xs sm:text-sm font-medium text-gray:600'>Credit Left : {credit}</p>
                    
                </button>
                <p className='text-gray-600 max-sm:hidden pl-4'>Hi, {user.name}</p>
                <div className='relative group'>
                    <img src={profile} className='w-10 drop-shadow'/>
